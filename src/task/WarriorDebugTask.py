...
class WarriorDebugTask(TriggerTask, BaseMapleTask):
    ...
    def run(self):
        ...
        if self._anchor is None:
            # 首次:全屏中央区慢扫定位(需要名字牌,角色名必填)
            hit = anchor.find_anchor(frame, character_name, prev_anchor=None)
            if hit is None:
        ...
        body_center = (self._anchor[0], self._anchor[1] + cfg['名字牌到身体偏移'])
    ...
    def _draw_debug(self, frame, cfg, facing, in_zone, mobs):
        ...
        if overlay is None:
            return
        anchor_xy = self._anchor
        body_center = (anchor_xy[0], anchor_xy[1] + cfg['名字牌到身体偏移'])
        zone = farm_logic.warrior_attack_zone(
