...
CONFIG_GROUPS = [
    ...
    ('角色定位', ['角色名', '名字牌到身体偏移(像素)', '锚点搜索区宽(比例)', '锚点搜索区高(比例)',
                ...
                '身份保鲜(秒)', '身份复验开关', '身份复验间隔(秒)', '丢锚立即重扫开关', '丢锚唯一框接管开关']),
    ('战斗细节', ['转向冷却(秒)', '受击防抖(秒)', '硬直抑制窗(秒)', '朝向纠正开关', '朝向观测开关']),
    ('挂机辅助', ['喂宠物开关', '喂宠物间隔(秒)', '补BUFF开关', '补BUFF间隔(秒)', '补BUFF列表', '坐椅开关',
                 '坐椅延迟(秒)', '经验停滞上限(分钟)']),
...
class MapleFarmTask(TriggerTask, BaseMapleTask):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.config_groups = CONFIG_GROUPS  # 展开区配置项功能分组(UI 展示用,见 ok/gui/tasks/ConfigCard.py)
        ...
        self.config_description.update({
            ...
            '二连击开关': '检测模式下区内有怪时,攻击键与副攻击键背靠背连按两次(先攻击键、立即副攻击键,中间不插入垫步等任何指令),第一次攻击打不死、怪靠近后被角色/宠物遮挡导致短暂检测不到时,第二下补刀。开启后跳过「攻击前垫步」。需「本开关开 且 设置页「游戏按键」已绑定「副攻击键(可留空)」」同时满足才生效,任缺其一不启用。超过两下才死的怪不适合挂机',
            ...
            '补BUFF开关': '到点且攻击区内无怪时自动按 BUFF 快捷键补上(检测模式才有攻击区状态;定频模式不补)。攻击区内有怪则优先解决怪物,补BUFF顺延到下一拍;触发那一拍暂停攻击/寻怪,只按 BUFF 键',
            '补BUFF间隔(秒)': '相邻两个 BUFF 按键之间的最小间隔(秒),防技能前摇吞键。多个 BUFF 同时到期时逐拍补、间隔至少此值;间隔期间攻击区仍无怪才会推进队列',
            '补BUFF列表': '点「Modify Buffs」按钮打开编辑区:每个 BUFF 填 名称/按键/间隔秒,可添加/删除/排序。每项独立计时,到点才补;间隔最小 1 秒',
    ...
    def _register_config_types(self):
        ...
        self.config_type['攻击区形状'] = {'type': 'drop_down',
                                          'options': ['单体(面朝)', '群体(对称)']}
        self.config_type['补BUFF列表'] = {'type': 'buff_list'}
