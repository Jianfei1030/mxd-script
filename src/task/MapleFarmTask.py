...
class MapleFarmTask(TriggerTask, BaseMapleTask):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.config_description.update({
            ...
            '群攻怪数阈值': '接敌区内怪数达到此值就改用群攻(前后双向命中),那一拍不转向、也不按单体攻击键。群攻不另设节拍,和单体共用「攻击间隔(秒)」——到点了看区内怪数决定按哪个键。需要先在设置页「游戏按键」绑定「群攻键(可留空)」,留空则本项无效。默认 3 的依据:2026-08-07 实测 800x200 接敌区里平均区内怪 0.36 只、有怪帧仅 28%、两侧都有仅 6%——阈值设 2 会让「一左一右两只」这种常见局面天天触发群攻,而那种局面转向一次就够。3 是「确实被围住了」的起点。实跑后按决策日志里的 区内=N 分布回调',
