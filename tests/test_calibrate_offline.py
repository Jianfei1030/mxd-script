...
class TestCalibrateOffline(unittest.TestCase):
    ...
    def test_locate_body_with_offset(self):
        """名字牌中心 (≈1165,875) + 偏移 90 → 身体中心 (≈1165,965)。"""
        with patch('src.detect.anchor._get_ocr', return_value=self.fake_ocr):
            body = locate_body(self.frame, 'Yufeng咕咕', offset=90)
        self.assertAlmostEqual(body[0], 1165, delta=1)
        self.assertAlmostEqual(body[1], 965, delta=1)

    def test_locate_body_miss_returns_none(self):
        with patch('src.detect.anchor._get_ocr', return_value=self.fake_ocr):
            body = locate_body(self.frame, '不存在的名字')
