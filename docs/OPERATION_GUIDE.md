...
- **90% 站桩**:你最终就是站桩挂机,模型要学的正是"站桩视角下怪长什么样"。站桩时**不要攻击**,让怪自然从屏幕两侧走进走出
- **10% 小范围变位**:换 1~2 次站桩点(上下/左右平移一段),让背景与怪的相对位置有变化,防模型过拟合单一背景
- 移动时不要开采集;站定再采。采集期间角色静止,只有怪在动

**采多久 / 采多少**

- **1 个刷怪点:50 帧 ≈ 2 分钟**(2 秒间隔 × 50 = 100 秒,覆盖 1~2 个完整刷怪周期)
- **总量建议:3 个点 × 50 帧 = 150 帧**(原管线标准)。想更稳 → 3~4 个点 × 50~80 帧
