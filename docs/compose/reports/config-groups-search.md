...
「实时触发」tab 的「自动打怪」任务卡片展开区新增两项纯 UI 能力：(1) 全部 65 个配置项按功能分 9 组（攻击/拾取/保命与药水/走位与朝向/寻怪/角色定位/战斗细节/挂机辅助/调试），组标题青绿粗体 + 组间虚线，组内容连续排列；(2) 展开区顶部搜索框，输入关键字做**宽松匹配**（键名+描述子串、忽略大小写、空输入全显示），匹配项保留、组标题随组内匹配显隐，清空恢复全部分组。
...
- **`ok/gui/tasks/ConfigCard.py`**（`TaskCard` 的父类，仅任务卡片使用）：消费 `task.config_groups`。`__ordered_config_keys` 按组序重排渲染键（组连续、每组只插一个标题）；`__maybe_add_group_header` 在组首插入标题；`__maybe_add_search_box` 建搜索框（仅在有分组的卡片）；`__apply_search_filter` 收到 textChanged 后按纯函数结果 setVisible。全部走空检查——无分组的任务跳过所有新逻辑。
...
- `tests/test_config_card_ui.py`（6 用例，offscreen QApplication）：真实 MapleFarmTask + ConfigCard 渲染——搜索框存在、9 组标题、过滤后键与组标题显隐、清空恢复、无分组任务无搜索框；grab() 渲染图存 `screenshots/e2e/config_groups/`。
- 全量回归：587 测试全绿（12 skip），全源码 py_compile 通过。
- E2E：offscreen 截图经视觉模型验收通过（9 组标题 + 顶部搜索框 + 过滤后跨组匹配）；真实 GUI（提权启动）无崩溃。
