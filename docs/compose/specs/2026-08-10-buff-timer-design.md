...
| 补BUFF列表 | list | [] | **可添加配置区**(2026-08-12 由 str 改为 list):GUI 点「Modify Buffs」弹编辑区,每项填 名称/按键/间隔秒 三字段,支持添加/删除/排序。存储为 list,元素 `名称=按键:间隔秒`。例:`['魔法盾=q:180', '狂暴=w:300']` |

- 放任务配置（非全局按键）:BUFF 列表是「按键+时间」组合策略,与喂宠物/坐椅同属挂机辅助
- 解析走 `farm_logic.parse_buff_config`（接受 str 逗号分隔或 list 两种输入,见 [S4]）
- GUI 控件 `ok/gui/tasks/LabelAndBuffList.py`:`buff_list` 配置类型,AddBuffDialog 名称/按键/间隔 三字段表单;老字符串配置会被 `verify_config` 类型校验重置为 `[]`(用户重新添加即可)
