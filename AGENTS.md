...
- 提交前跑全量测试确认无回归
- **开发过程中的视觉任务(如 agent 看截图、验证 overlay 画面、读图像类证据)使用 `mimo v2.5` 模型(from opencode go)**——需要 AI 视觉能力时指定该模型,不要用无视觉的默认模型
- ⚠️ 上述仅指**开发过程**中 AI 的视觉能力,与游戏**运行时**的实际视觉(怪物检测 YOLO / OCR)/ **训练**用的视觉模型无关——那些仍按 §3.1/§4 走