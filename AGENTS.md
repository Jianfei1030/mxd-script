...
- **功能**：「实时触发」→「自动打怪」卡片展开区：全部配置项按功能分 9 组（攻击/拾取/保命与药水/走位与朝向/寻怪/角色定位/战斗细节/挂机辅助/调试），组标题青绿粗体 + 组间虚线；展开区顶部有搜索框，输入关键字**宽松匹配**（键名+描述子串、忽略大小写、空输入全显示），匹配项保留、组标题随组内匹配显隐；清空恢复全部分组。仅 UI 层，不改任何任务执行/配置读写逻辑
...
- **测试**：`tests/test_config_groups.py`（CONFIG_GROUPS 覆盖 DEFAULT_CONFIG 全部键且不重复、唯一组名、matches/visible_keys/visible_groups 纯函数）；`tests/test_config_card_ui.py`（offscreen 渲染：搜索框存在/组标题数/过滤/清空恢复/无分组任务无搜索框；grab 渲染图存 `screenshots/e2e/config_groups/`）
- **E2E 验收（2026-08-11）**：offscreen 渲染截图经视觉模型验收通过（9 组标题 + 顶部搜索框 + 过滤后跨组匹配）；真实 GUI 启动无崩溃。⚠️ agent 受限窗口站无法截图提权 GUI（PyAutoGUI 全屏与 `_e2e_capture.py` 都抓到空白），交互类 E2E 走 offscreen grab + 断言
